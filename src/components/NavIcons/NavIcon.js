    ...
    return (
        <Fragment>
            <div className={`tooltipNavIcons ${colorMode === 'dark' ? "tooltipNavIconsDarkMode" : ""} ${type === 'imprint' ? "hideImprint" : ""}`}>
                <img draggable="false" className={`navIcon ${route === type ? "navIconSelected" : ""} ${colorMode === 'dark' ? "navIconDarkMode" : ""}`} alt="HomeNav Icon" src={`./icons/${type}.png`} onClick={() => onRouteChange(type)} />
